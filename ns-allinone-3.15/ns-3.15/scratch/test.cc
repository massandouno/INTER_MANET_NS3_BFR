...
//#include "ns3/inter-manet-module.h"
...
main (int argc, char *argv[])
{
  ...
  mobilityL.Install (wifiNodesL);
  mobilityL.Install (wifiNodesR.Get(1));
...
*/
  ...
  AodvHelper aodv;
  OlsrHelper olsr;
  //InterMANETHelper dsdv;
  DsrHelper dsr;
  Ipv4StaticRoutingHelper staticRouting;
  
  //InterMANETRoutingHelper list;
  //list.Add (aodv, 10);
  //list.Add (olsr, 100);
  //InterMANETRoutingHelper listL;
  //listL.Add (aodv, 10);
  //InterMANETRoutingHelper listR;
  //listR.Add (olsr, 10);
  ...
  ApplicationContainer serverApps = echoServer.Install (wifiNodesL.Get (1));
  serverApps.Start (Seconds (1.0));
  serverApps.Stop (Seconds (300.0));

  UdpEchoClientHelper echoClient (leftInterface.GetAddress (1), 9);
  echoClient.SetAttribute ("MaxPackets", UintegerValue (1000));
  echoClient.SetAttribute ("Interval", TimeValue (Seconds (0.08)));
  echoClient.SetAttribute ("PacketSize", UintegerValue (1024));

  ApplicationContainer clientApps = 
  echoClient.Install (wifiNodesR.Get (1));
